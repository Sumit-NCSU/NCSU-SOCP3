...
public class Utils {
	public static void updateExpertise(final double[] query, final double[] answer, double[] expertise) {
	...
	public static void updateSociability(final double[] query, final double[] answer, int distance2tail,
			double[] sociability) {
	...
	public static boolean isExpertiseMatch(final double[] expertise, final double[] query) {
		assert expertise.length == query.length;
		for (int i = 0; i < expertise.length; i++) {
			if (expertise[i] / query[i] < 0.5)
				return false;
		}
		return true;
	}

	public static double[] genAnswer(final double[] expertise, final double[] query) {
